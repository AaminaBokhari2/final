...
class AIPresentationCoordinatorAgent:
    """
    Coordinates AI-powered presentation creation with robust fallback mechanisms
    """
    ...
    def __init__(self, groq_client):
        ...
        self.themes = {
            ...
            "academic": {
                "colors": ["#2b5876", "#4e4376", "#4b79a1"],
                "fonts": {"title": "Times New Roman", "body": "Calibri"},
                "style": "Academic presentation style"
            }
        }

    def create_presentation(self, topic: str, audience: str = "general",
                          duration: int = 10, theme: str = "professional") -> Tuple[PresentationData, Dict, Dict]:
        """
        Main method to create a presentation
        ...
        """
        try:
            self.logger.info(f"Starting presentation creation: {topic}")
            
            # Calculate slide count (2 minutes per slide)
            slides_count = max(3, min(duration // 2, 12))
            
            # First try AI generation
            presentation_data = None
            if self.client:
                try:
                    presentation_data = self._generate_ai_content(topic, audience, slides_count)
                except Exception as e:
            ...
            return (
                ...
                self._generate_design_guidelines(theme, topic),
                self._assess_quality(slides)
            )
            
        except Exception as e:
            self.logger.error(f"Presentation creation failed: {str(e)}")
            return self._create_fallback_presentation(topic, theme)

    def _generate_ai_content(self, topic: str, audience: str, slides_count: int) -> Optional[Dict]:
        """Generate presentation content using AI"""
        prompt = f"""Create a {slides_count}-slide presentation about "{topic}" for {audience} audience.
...
- Maximum 5 bullet points per slide"""
    ...
    def _create_fallback_content(self, topic: str, slides_count: int) -> Dict:
        ...
        clean_topic = topic.strip() or "Presentation Topic"
    ...
    def _create_fallback_presentation(self, topic: str, theme: str) -> Tuple[PresentationData, Dict, Dict]:
        """Generate complete fallback presentation"""
        ...
        slides = [
            SlideContent(
                ...
                notes="Automatically generated content"
    ...
    def _create_slides(self, presentation_data: Dict, theme: str) -> List[SlideContent]:
        """Convert raw data to SlideContent objects"""
        return [
            SlideContent(
    ...
    def _generate_design_guidelines(self, theme: str, topic: str) -> Dict:
        ...
        return {
            ...
            "recommendations": [
                f"Use {theme_config['style']}",
                "Maintain consistent spacing",
                "Limit text per slide"
            ]
        }

    def _assess_quality(self, slides: List[SlideContent]) -> Dict:
        ...
        for i, slide in enumerate(slides):
            if not slide.content:
                issues.append(f"Slide {i+1} has no content")
                score -= 10
            elif len(slide.content) > 5:
                suggestions.append(f"Reduce bullets on slide {i+1}")
                score -= 5
        ...
        return {
            ...
            "rating": "Good" if score >= 80 else "Needs Improvement"
        }

    def create_pptx_file(self, presentation_data: PresentationData, output_path: str) -> bool:
        ...
        try:
            ...
            # Add slides
            for slide_content in presentation_data.slides:
            ...
            self.logger.error(f"PPTX generation failed: {str(e)}")
            return False
...
class PresentationAgent:
    def __init__(self, slidegpt_api_key: str = None):
        """Initialize SlideGPT presentation agent with fallback"""
        self.api_key = slidegpt_api_key or os.getenv("SLIDEGPT_API_KEY")
        self.base_url = "https://api.slidegpt.io/v1"
        self.session = requests.Session()
        self.session.headers.update({
    ...
    def generate_presentation(self, document_text: str, output_path: str, max_slides: int = 10, generate_images: bool = False) -> Dict:
        """Generate presentation with robust error handling"""
        if not self.api_key:
            return {
                "status": "error",
                "error": "SlideGPT API key not configured",
                "fallback": True
            }

        payload = {
            "content": document_text[:5000],  # Limit content length
            "slides": min(max(max_slides, 5), 20),  # Clamp between 5-20
    ...
    def download_presentation(self, presentation_id: str, output_path: str) -> Dict:
